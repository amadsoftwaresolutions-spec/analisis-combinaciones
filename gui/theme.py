"""
gui/theme.py — Nova shared component library.
Clean, minimal widget factories for the entire app.
"""
...
def card(parent, **kw) -> ctk.CTkFrame:
    """Standard card — rounded, 1px border, CLR_CARD background."""
    defaults = dict(fg_color=CLR_CARD, corner_radius=10,
...
def primary_btn(parent, text: str, command=None,
    ...
    return ctk.CTkButton(parent,
                         ...
                         fg_color=CLR_ACCENT, hover_color=CLR_BTN_HOVER,
                         text_color="#0d0d10",
                         corner_radius=8,
...
def stat_card(parent, title: str, value: str,
    ...
    if icon:
        tk.Label(f, text=icon,
                 font=("Segoe UI", 16),
                 fg=color, bg=CLR_CARD2).pack(pady=(14, 0))
    tk.Label(f, text=value,
             font=("Segoe UI", 22, "bold"),
             fg=color, bg=CLR_CARD2).pack(pady=(6, 0))
    tk.Label(f, text=title,
             font=("Segoe UI", 9),
             fg=CLR_TEXT_DIM, bg=CLR_CARD2).pack(pady=(2, 14))
    return f
...
def empty_state(parent, icon: str = "◎",
    ...
    tk.Label(f, text=icon,
             font=("Segoe UI", 36),
             fg=CLR_TEXT_DIM, bg=bg).pack(pady=(40, 8))
    tk.Label(f, text=title,
             font=("Segoe UI", 15, "bold"),
             fg=CLR_TEXT_MID, bg=bg).pack()
    tk.Label(f, text=subtitle,
             font=("Segoe UI", 10),
             fg=CLR_TEXT_DIM, bg=bg).pack(pady=(4, 40))
    return f
